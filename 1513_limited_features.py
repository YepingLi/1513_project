...
selected_features = ['AST', 'ALT', 'NIMAP', 'NIDiasABP', 'NISysABP']

train_data = pd.read_csv("ml_train.csv")
test_data = pd.read_csv("ml_test.csv")
val_data = pd.read_csv("ml_val.csv")

# Modify data loading to only use selected features
train_data_stat = train_data[selected_features].values
test_data_stat = test_data[selected_features].values
val_data_stat = val_data[selected_features].values
...
class NeuralNetwork(torch.nn.Module):
    def __init__(self):
        ...
        # Change input size to 5 features
        self.fc1 = torch.nn.Linear(5, 150)
